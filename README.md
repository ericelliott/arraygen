...
As you can see, the synchronous operations are probably better handled with native `array.slice()`. It's almost certainly less typing, anyway. This slice API is just a convenient way to pre-slice an array while you're grabbing the iterator. I would just use the built-in `slice()`, but the API has always bothered me a little.
